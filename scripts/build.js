...
async function uploadToAWS(buildEnv) {
  ...
  return new Promise((resolve, reject) => {
    exec(`aws s3 sync .next "s3://aimementoring${environment}/website/_next" --acl public-read --add-header="Cache-Control:public,max-age=31557600" --delete`, (error, stdout, stderr) => {
      if (error) reject(error);
      if (stderr) console.info(stderr);
      resolve(stdout);
    });
