...
module.exports = async (migration) => {
  ...
  const bodyFromCraft = (content, changeCount) => {
    ...
    // eslint-disable-next-line no-console
    // console.log(changeCount, 'craft body ---> ',
    // entriesCraft.post.map((ele) => (ele !== 'paragraph'
    // && ele !== undefined ? ele.paragraph : null)));
    const test = entriesCraft.post.map((ele) => (ele.paragraph !== 'paragraph' && ele.paragraph !== undefined ? ele.paragraph : null));

    // console.log('TCL: bodyFromCraft -> test', removeHTML(test[0]),
    // removeHTML(test[1]), removeHTML(test[4]));
