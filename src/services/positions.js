...
export async function loadPositions(filter, fields) {
  return airtableFetchRecords(config, filter, fields);
}

export async function findJob(id, currentSite) {
...
export async function loadUniversities() {
  return airtableFetchRecords(sitesConfig).then(options => options);
}
