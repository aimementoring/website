...
export default class GeneralMatrix extends PureComponent {
  static propTypes = {
  ...
  getMatrix = () => {
    ...
    return this.props.formData.map((formElement) => {
  ...
  renderHeadingOne = formElement => {
    ...
    return (
      <div className="wrap-md mb0 md-mb3 lg-mb3 clearfix" key={formElement.name}>
        ...
        {block.subtitle
          && <p className={`destroy-p ${block.subtitleStyle} ${block.textAlign} mx-auto`}>{block.subtitle}</p>}
      </div>
    );
  }
  ...
  renderHeadingThree = formElement => {
    const block = formElement.content;
    return (
      <div className="wrap-md" key={formElement.name}><h4 className="f-15 c-grey feature-font-family regular">{block.text}</h4></div>
    );
  }
  ...
  renderBanner = formElement => {
    ...
    return (
      <div
        ...
        style={{ backgroundImage: `url('${block.image[0].url}}`, backgroundPosition: block.imagePosition || "default('0% 25%')" }}>
  ...
  renderSignature = formElement => {
    ...
    return (
      <div key={formElement.name} className="wrap-md mb3">
        ...
        <span className="f-14"><strong>{block.authorTitle}</strong></span>
      </div>
    );
  }
    ...
    return (
      <div key={formElement.name} className="wrap w100 clearfix">
        <div className={`${unHideClass} left hide"`}>
          <Link to={block.previousButtonUrl} className="block px2 left c-grey bold px2 feature-font-family">
