...
export async function airtableFetchRecord(config, id) {
  Airtable.configure({
    endpointUrl: 'https://api.airtable.com',
    apiKey: process.env.REACT_APP_AIRTABLE_API_KEY,
  });
...
export async function airtableUpdateRecord(config, id, data) {
  Airtable.configure({
    endpointUrl: 'https://api.airtable.com',
    apiKey: process.env.REACT_APP_AIRTABLE_API_KEY,
  });
...
export async function loadList(filter, fields, fieldsMapping, config) {
  const fieldsAttributes = fields && fields.length ? fields.map(field => fieldsMapping[field]) : Object.keys(fieldsMapping).map(key => fieldsMapping[key]);
