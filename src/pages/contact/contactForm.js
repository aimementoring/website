...
const ContactForm = () => {
  ...
  const fetchFormElements = () => {
    getEntries('/contact')
      .then(entries => setFormElements(entries.formBuilder))
      .catch(err => bugsnagClient.notify(new Error(`Error getting the form of contact: ${err}`)));
  };
